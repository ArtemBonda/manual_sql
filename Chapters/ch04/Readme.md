...
* decimal(s,p)
  ...
  * 12.3456 => 
    * масштаб = 4 (3456), точность = 6(общее количество символов)
...
```
Или использовать альтернативный формат, предлагаемый стандартом ISO 8601:
```
...
* Например, для получения текущей временной отметки с точностью до одного часа
```
...
```
Или воспользоваться оператором &&, который проверяет наличие общих элементов у массива
```
SELECT * FROM pilots
...
* `json` - сохранение значений происходит быстрее, потому что они записываются в том виде, в котором были введены.
...
UPDATE pilot_hobbies
    ...
    WHERE pilot_name='Boris';
```
Например можно вставить неправильное згачение или сделать опечатку в ключе, чтобы изменить или удалить этот ключ 
можно использовать `jsonb_delete()` или `jsonb_delete_path()`
```
update pilot_hobbies
